...
@Suite
struct MTC_Receiver_Receiver_Tests {
    @Test
    func mtcReceiver_Default() {
    ...
    @Test
    func mtcReceiver_Init_Arguments() {
...
extension MTC_Receiver_Receiver_Tests {
    ...
    func mtcReceiver_Handlers_FullFrameMessage() async throws {
        ...
        final class Receiver {
            ...
            func set(timecode: Timecode?) {
                self.timecode = timecode
            }

            var mType: MTCMessageType?
            func set(mType: MTCMessageType?) {
                self.mType = mType
            }

            var direction: MTCDirection?
            func set(direction: MTCDirection?) {
                self.direction = direction
            }

            var isFrameChanged: Bool?
            func set(isFrameChanged: Bool?) {
                self.isFrameChanged = isFrameChanged
            }

            var state: MTCReceiver.State?
            func set(state: MTCReceiver.State?) {
                self.state = state
            }
